...
fun timeForHalfWay(
    ...
    return when (val halfPath = (t1 * v1 + t2 * v2 + t3 * v3) / 2.0) {
        in 0.0..t1 * v1 -> halfPath / v1
        in t1 * v1..t1 * v1 + t2 * v2 -> t1 + (halfPath - t1 * v1) / v2
        else -> (halfPath - t1 * v1 - t2 * v2) / v3 + t1 + t2
    }
...
//    else false
//
class Car(color: String, wheels: Int, powerOfEngine: Int) {
    val averagePower = powerOfEngine * 1.0 / wheels
...
fun main() {
    val a = 8
    if (a in 0..10) println(87686)
    val blueCar = Car("blue", 4, 1000)
    println(blueCar.averagePower)
    val yellowCar = Car("yellow", 6, 200000)
    println(yellowCar.averagePower)
